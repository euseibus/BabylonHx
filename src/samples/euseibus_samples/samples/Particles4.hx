...
class Particles4 {
	...
	public function new(scene:Scene) {
		...
		particleSystem.particleTexture = new Texture("assets/models/Flare.png", scene);
		...
		particleSystem.maxLifeTime = 100.0;
		...
		// Speed
		particleSystem.minEmitPower = 0.51;
		...
		scene.registerBeforeRender(function(scene:Scene, es:Null<EventState>) {
			...
			particleSystem.direction1 = new Vector3(px / 100 , py / 100, pz / 100);
			particleSystem.direction2 = new Vector3(pz / 100, px / 100, py / 100);
		...
		scene.getEngine().runRenderLoop(function () {
			var fps:Float = 0.0;
			scene.render();
			fps=scene.getCameraByName("Camera").getEngine().getFps();
			trace("FPS: "+fps);
		});
	}
